...
Nygard由来の構成に、機械検証できる `Status` と `Proposed-on` を追加する。状態はNygardとMADRを参考に5種類へ固定する。提案30日以上でエラー、日付の必須化、大文字小文字、見出し順序、置換先の別フィールド化は**このツールの運用規約**であり、ADR一般の標準ではない。MADR完全互換とは称さない。
