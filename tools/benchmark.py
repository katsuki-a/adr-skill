...
def main():
    ...
    sample = (ROOT / 'tests/fixtures/0001-use-standard-library.md').read_text(encoding='utf-8')
    ...
    with tempfile.TemporaryDirectory() as temp:
        ...
        for count in (1, 100, 1000):
            ...
            for n in range(1, count + 1):
                (folder / ('%04d-example.md' % n)).write_text(sample.replace('# 0001.', '# %04d.' % n, 1), encoding='utf-8')
        ...
        for n in range(args.repeats):
            ...
            created.append(ms)
            draft = next(new_dir.glob('*.md'))
            draft.write_text(sample.replace('## Status\n\n', '## Status\n\n\n'), encoding='utf-8')
    ...
    if importlib.util.find_spec('tiktoken'):
        ...
        template = (SKILL / 'assets/adr.md').read_text().replace('{{id}}', '0001').replace('{{title}}', 'ADR検証方式').replace('{{proposed_on}}', '2026-09-13')
        command_text = "python3 scripts/adr.py new --directory docs/adr --slug example --title 'ADR検証方式'\npython3 scripts/adr.py format docs/adr/0001-example.md"
        components = {'skill': tokens((SKILL / 'SKILL.md').read_text()), 'generated_template_and_path': tokens('docs/adr/0001-example.md\n' + template), 'validation_stdout': tokens(format_output), 'command_text': tokens(command_text)}
